...
class ClientController extends AbstractController
{
    ...
    public function autocomplete(Request $request): Response
    {
        ...
        $constraints = new Assert\Collection([
            ...
            'config' => [
                ...
                new Assert\Callback(function ($object, ExecutionContextInterface $context, $payload) {
                    if (strpos(base64_decode($object), 'module.exports') === false) {
        ...
        $autocomplete->setConfig(base64_decode($data['config']));
    ...
    public function compile(Request $request): Response
    {
        ...
        $compile = new Compile();
        $compile->setConfig(base64_decode($data['config']));
        $compile->setNonce($data['nonce']);
        $compile->setSite($data['site']);
        $compile->setCss(base64_decode($data['css']));
        $compile->setContent(base64_decode($data['content']));
    ...
    #[Route('/pull_queue', name: 'pull_queue', methods: ['GET'])]
    public function pull_queue(CacheInterface $cache, Request $request): Response
    {
        /** @var null|Autocomplete|Compile $entity */
        $entity = null;
    ...
    #[Route('/github_action', name: 'github_action', methods: ['GET'])]
    public function action(Request $request): Response
    {
        ...
        $run_id = $entity->getRun()->getId();

        $repos = $this->getParameter('app.worker.repos');
        
        // redirect to the github action page
        $endpoint = sprintf(
